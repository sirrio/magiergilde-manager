...
<template>
  <dialog ref="modalGameUpdate" class="modal">
    <div class="modal-box">
      <form method="dialog">
        <button class="btn btn-sm btn-circle btn-ghost absolute top-2 right-2">✕</button>
      </form>
      <form class="flex flex-col gap-3" @submit.prevent="clickUpdateNewGame()">
        <h3 class="mb-6 text-lg font-bold">Update your game</h3>
        ...
        <label class="form-control w-full">
          ...
          <input v-model="form.title" placeholder="Peters greatest game" type="text" class="input" />
        </label>
        ...
        <label class="form-control">
          <div class="label">
            <span class="label-text">How many sessions did you play? <span class="text-xs italic">(Series)</span></span>
          </div>
          <input v-model="form.sessions" type="number" min="0" placeholder="1" class="input" />
        </label>
